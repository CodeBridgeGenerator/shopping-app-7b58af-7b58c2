...
import CustomerManagementDatatable from "./CustomerManagementDataTable";
import CustomerManagementEditDialogComponent from "./CustomerManagementEditDialogComponent";
import CustomerManagementCreateDialogComponent from "./CustomerManagementCreateDialogComponent";
import CustomerManagementFakerDialogComponent from "./CustomerManagementFakerDialogComponent";
import CustomerManagementSeederDialogComponent from "./CustomerManagementSeederDialogComponent";
...
const CustomerManagementPage = (props) => {
    ...
    const filename = "customerManagement";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("customerManagement");
    ...
    useEffect(() => {
        ...
        client
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Customer Management", type: "error", message: error.message || "Failed get Customer Management" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("customerManagement").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Customer Management", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("customerManagement").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/app/customerManagement/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="customerManagement-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-12" role="customerManagement-datatable">
                    <CustomerManagementDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <CustomerManagementEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <CustomerManagementCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <CustomerManagementFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <CustomerManagementSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(CustomerManagementPage);
