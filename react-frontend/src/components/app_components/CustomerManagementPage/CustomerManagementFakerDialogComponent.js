...
import customerManagementFakerFactory from "./customerManagementFakerFactory";
...
const CustomerManagementFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = customerManagementFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("customerManagement").create(elem));
...
export default connect(mapState, mapDispatch)(CustomerManagementFakerDialogComponent);
